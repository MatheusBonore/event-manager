<x-app-layout>
	...
	@push('scripts')
		<script>
			...
			document.getElementById('event-form').addEventListener('submit', function(event) {
				...
				.then(data => {
					if (data.errors) {
						...
						showToast(data.event.event);
					}
				})
				.catch(error => console.error('An error occurred while creating the event.'));
			});
			...
			function showToast(event) {
				...
				document.getElementById('toast-link').setAttribute('href', `/events?event=${event}`);
	...
	<div id="toast-success" class="fixed right-5 bottom-5 hidden max-w-xs p-4 mb-4 text-gray-500 bg-white rounded-lg shadow-lg dark:text-gray-400 dark:bg-gray-800" role="alert">
		<div class="flex items-center w-full">
			...
			<div class="mx-3">
				<div class="font-normal">successfully created event.</div>

				<div>
					<a href="" id="toast-link" class="inline-flex items-center font-medium text-blue-600 dark:text-blue-500 hover:underline">
						See event
						<svg class="rtl:rotate-180 w-3.5 h-3.5 ms-2" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 10">
							<path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M1 5h12m0 0L9 1m4 4L9 9"/>
						</svg>
